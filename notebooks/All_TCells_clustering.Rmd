...
## Over/Under-Representation of gRNAs in Clusters
...
```{r chisq_test_barplot_selected}
cluster_indx <- paste0("Cluster ", c(1, 2, 5, 6))
marker_indx <- c("CBLB", "CD5", "CDKN1B", "SOCS1", "TCEB2",
                 "RASA2", "LCP2", "CD3D", "NonTarget")
marker_indx <- marker_indx[length(marker_indx):1]
selected_df <- as.data.frame(t(chisq_stdres[cluster_indx, marker_indx]))
selected_df <- selected_df %>% mutate(target = factor(marker_indx, levels = marker_indx))

combined_selected_df <- reshape2::melt(selected_df, id.var = "target",
                                       variable.name = "cluster", value.name = "stdres")
ggplot(combined_selected_df,
       aes(y = target, x = stdres, fill = stdres)) +
  facet_wrap(~ cluster) +
  geom_col() +
  scale_fill_gradient2(low = scales::muted("blue"), high = scales::muted("red")) +
  labs(x = "chi-square stardardized residual") +
