...
gsfa_folder <- "gsfa_output_tmm_female_white/corrected/"
guide <- "top5_lof_mis"
...
print(knitr::kable(rbind(num_per_marker[, 1:5], num_per_marker[, 6:10]),
                   caption = "Number of samples that harbor each type of mutation (out of all 573)") %>%
    kable_styling() %>% scroll_box(width = '100%'))
```
...
qplot(unlist(beta_pip_matrix), -log10(unlist(heatmap_matrix)),
            ...
            main = "20 factors x 10 conditions\n(red line at y=5, omiting 1 data point at y=26)") +
...
```{r flash_tb}
flash_obj <-
  readRDS(paste0(wkdir, "flash_output/BRCA_TMM_w_mutation.female_white_only.top10k_var_scaled.corrected.flashier_obj_k20.rds"))
