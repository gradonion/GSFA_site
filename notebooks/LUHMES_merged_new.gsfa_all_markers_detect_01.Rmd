...
GO category: Biological Process (non-redundant).

```{r}
gsfa.Web_ORA <-
  readRDS(paste0(wkdir, gsfa_folder,
                 "ORA_GO/All.gibbs_obj_k20.svd.WebGestalt_ORA.cutoff_0.95.rds"))
```

```{r}
enrich_list <- gsfa.Web_ORA
signif_num <- rep(0, length(enrich_list))
for (i in 1:length(enrich_list)){
  if (is.null(enrich_list[[i]])){ next }
  signif_tb <- enrich_list[[i]] %>% filter(enrichmentRatio >= 2)
  if (nrow(signif_tb) == 0){ next }
  signif_tb <- signif_tb %>% mutate(GeneRatio = paste(overlap, size, sep = "/")) %>%
    mutate(GO_term = paste0("[", geneSet, "](", link, ")")) %>%
    select(GO_term, description, enrichmentRatio, pValue, FDR, GeneRatio) %>%
