...
marker_genes <- c("MAP2", "DCX", "STMN2", "NES", "TP53", "CDK4")
plot.lst <- list()
...
```{r}
lfsr_cutoff <- 0.05
lfsr_mat <- readRDS(paste0(wkdir, gsfa_folder, guide, ".gibbs_obj_k20.svd.lfsr_mat.group0.rds"))
...
```{r}
lfsr_cutoff <- 0.05
lfsr_mat <- readRDS(paste0(wkdir, gsfa_folder, guide, ".gibbs_obj_k20.svd.lfsr_mat.group1.rds"))
lfsr_signif_num <- colSums(lfsr_mat < lfsr_cutoff)
print_signif_tb(lfsr_signif_num, paste0("GSFA, # of genes that passed LFSR cutoff of 0.05:"))
```
...
lfsr_mat <- readRDS(paste0(wkdir, gsfa_folder, "All.gibbs_obj_k20.svd.lfsr_mat.group0.rds"))
...
for (m in signif_markers){
  effect_size_mat[, m] <-
    colSums(beta_mat[m, ] * t(gene_loadings[interest_df$gene_name, ]))
}
```
...
early_p1 <- dotplot_effectsize(effect_size_mat[interest_df1$protein_name, ],
                   selected_lfsr_mat,
...
early_p2 <- dotplot_effectsize(effect_size_mat[interest_df2$protein_name, ],
                   selected_lfsr_mat,
                   reorder_markers = signif_markers,
                   color_lgd_title = "Summarized effect") +
  theme(axis.text.y = element_text(hjust = 1)) +
...
```{r Late stage cells}
interest_df <- rbind(interest_df1, interest_df2)
lfsr_mat <- readRDS(paste0(wkdir, gsfa_folder, "All.gibbs_obj_k20.svd.lfsr_mat.group1.rds"))
stopifnot(rownames(lfsr_mat) == genes_df$ID)
rownames(lfsr_mat) <- genes_df$Name
...
selected_lfsr_mat <- lfsr_mat[interest_df1$gene_name, signif_markers]
rownames(selected_lfsr_mat) <- interest_df1$protein_name
late_p1 <- dotplot_effectsize(effect_size_mat[interest_df1$protein_name, ],
                   selected_lfsr_mat,
                   reorder_markers = signif_markers,
                   color_lgd_title = "Summarized effect") +
  theme(axis.text.y = element_text(hjust = 1)) +
...
```{r fig.width=5, fig.height=6}
selected_lfsr_mat <- lfsr_mat[interest_df2$gene_name, signif_markers]
late_p2 <- dotplot_effectsize(effect_size_mat[interest_df2$protein_name, ],
                   selected_lfsr_mat,
                   reorder_markers = signif_markers,
                   color_lgd_title = "Summarized effect") +
  theme(axis.text.y = element_text(hjust = 1)) +
