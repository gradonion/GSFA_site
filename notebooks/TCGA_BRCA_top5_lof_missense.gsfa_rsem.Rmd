...
subtitle: "-- Guides: LOF and Missense Mutations Separated"
...
print_signif_tb <- function(signif_num, caption_text){
  signif_num <- t(data.frame(KO = names(signif_num),
                             ...
                             row.names = NULL))
  print(knitr::kable(rbind(signif_num[, 1:5], signif_num[, 6:10]),
...
meta_file <- "processed_data/BRCA_metadata.female_white_only.top5_lof_mis.rds"
...
all_KOs.G_mat <- metadata[, 14:23]
KO_names <- colnames(all_KOs.G_mat)
nmarkers <- ncol(all_KOs.G_mat)

num_per_marker <- colSums(all_KOs.G_mat)
num_per_marker <- t(data.frame(Mutation = names(num_per_marker),
                               Num_cells = num_per_marker,
                               row.names = NULL))
print(knitr::kable(rbind(num_per_marker[, 1:5], num_per_marker[, 6:10]),
                   caption = "Number of samples that harbor each type of mutation (out of all 700)") %>%
    kable_styling() %>% scroll_box(width = '100%'))
```
...
9: angiogenesis   
11: translation   
15: blood vessel development; epidermal cell differentiation    
16: RNA splicing    
...
signif_num.df <- t(data.frame(Mutation_status = names(gsfa.go_enrich_permarker),
                              Signif_GO_terms = signif_num,
                              row.names = NULL))
knitr::kable(rbind(signif_num.df[, 1:6], signif_num.df[, 7:12]),
             caption = "Summary:") %>%
  kable_styling() %>% scroll_box(width = '100%')
```

### Correlation btw factors from different guided results

```{r}
selected_7.gibbs_PM <- readRDS(paste0(wkdir, gsfa_folder, "selected_7", ".gibbs_obj_k20.svd.PM.rds"))
plot_pairwise.corr_heatmap(input_mat_1 = All_KOs.gibbs_PM$Z_pm,
                           input_mat_2 = selected_7.gibbs_PM$Z_pm,
                           corr_type = "pearson",
                           name_1 = "Z_pm (Guided by LOF-Missense Mutations)",
                           name_2 = "Z_pm (Guided by All Mutations)")
```
