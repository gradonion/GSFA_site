...
[GSFA all markers result, NTC condition regressed out, singleton cells only](gsfa_all_markers_detect_01_singleton.html);

[GSFA all markers result, NTC condition regressed out](gsfa_all_markers_detect_01_noNTC.html);

[GSFA all markers result, NTC as one of the guides](gsfa_all_markers_detect_01.html);

[Unguided G=0 result](unguided_detect_01.html);

[FLASH result](flash_detect_01.html).
...
[GSFA all markers result](gsfa_all_markers_top8k_var.html);

Unguided G=0 result;

[FLASH result](flash_top8k_var.html).
...
## Gene discovery comparison using local false sign rate

For each factor analysis result, we estimated the KO effect on genes using the local false sign rate (LFSR), and compared the result with that from Wilcoxon DGE.
