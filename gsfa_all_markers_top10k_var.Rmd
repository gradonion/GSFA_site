...
```{r}
All_KOs.gibbs_PM <-
  readRDS(paste0(wkdir, "gsfa_output_top10k_var/All.corrected.gibbs_obj_k20.svd.PM.rds"))
All_KOs.gibbs_res_tb <- make_gibbs_res_tb(All_KOs.gibbs_PM, all_KOs.G_mat)
```

```{r svd_heatmap, fig.width=10}
...
```{r svd_pval, fig.width=10, fig.height=5}
summ_pvalues(unlist(heatmap_matrix[, -30]), title_text = "GSFA All KOs (TP53 excluded, SVD)")
```

## Random Initialization -- 1

```{r rand_1_heatmap, fig.width=10}
rand_1.gibbs_PM <- readRDS(paste0(wkdir,
                                  "gsfa_output_top10k_var/All.corrected.gibbs_obj_k20.rand_01.PM.rds"))
rand.gibbs_res_tb <- make_gibbs_res_tb(rand_1.gibbs_PM, all_KOs.G_mat)
rand.gibbs_res_tb <- rand.gibbs_res_tb %>% arrange(-sum_var)
heatmap_matrix <- rand.gibbs_res_tb %>% select(starts_with("pval"))
rownames(heatmap_matrix) <- rand.gibbs_res_tb$index
colnames(heatmap_matrix) <- colnames(all_KOs.G_mat)
ntc_inx <- which(colnames(heatmap_matrix) == "NONTARGETING")
heatmap_matrix <- heatmap_matrix[, c(colnames(heatmap_matrix)[ntc_inx],
                                     colnames(heatmap_matrix)[-ntc_inx])]
plot_pval_heatmap(heatmap_matrix,
                  factor_annot = rand.gibbs_res_tb$pi)
```

```{r rand_1_pval, fig.width=10, fig.height=5}
summ_pvalues(unlist(heatmap_matrix[, -30]), title_text = "GSFA All KOs (TP53 excluded, Random 1)")
```

## Random Initialization -- 2

```{r rand_2_heatmap, fig.width=10}
rand_2.gibbs_PM <- readRDS(paste0(wkdir,
                                  "gsfa_output_top10k_var/All.corrected.gibbs_obj_k20.rand_02.PM.rds"))
rand.gibbs_res_tb <- make_gibbs_res_tb(rand_2.gibbs_PM, all_KOs.G_mat)
rand.gibbs_res_tb <- rand.gibbs_res_tb %>% arrange(-sum_var)
heatmap_matrix <- rand.gibbs_res_tb %>% select(starts_with("pval"))
rownames(heatmap_matrix) <- rand.gibbs_res_tb$index
colnames(heatmap_matrix) <- colnames(all_KOs.G_mat)
...
```{r rand_2_pval, fig.width=10, fig.height=5}
summ_pvalues(unlist(heatmap_matrix[, -30]), title_text = "GSFA All KOs (TP53 excluded, Random 2)")
```
...
G0.gibbs_PM <- readRDS(paste0(wkdir,
                              "gsfa_output_top10k_var/G0.corrected.gibbs_obj_k20.svd.PM.rds"))
...
```{r go_enrich}
gsfa.go_enrich <-
  readRDS(paste0(wkdir, "gsfa_output_top10k_var/enrich_res/All_KOs.corrected.k20_svd.go_enrich.cutoff_0.5.rds"))
